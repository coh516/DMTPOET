...
function linePoints() {
	...
	this.getCycles = function() {		
		...
		this.getRootPath(this.lastBox, false, this.curBox);
		return cycles;		
	}
	...
	this.addRemoveContainer = function(hidden) {
		if (!self.show) {
			...
			if (!self.container.hiddenListContainer) {
				...
				//	self.container.layer.draw();
				/*
				self.container.traverseSubElements( function(e) { if (e.container) if (e.container.listContainer) {
					for (var i =0; i < e.container.listContainer.listItems.length; i++) {
						e.container.listContainer.listItems[i].setLinesFromObj();
					}
				}
				})
				*/
