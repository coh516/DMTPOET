...
initializeStage = function() {
	...
	getRootPaths = function() {
		...
		this.getRootPath = function(tb, lb, str) {
			for (var i in tb.linkedBoxes) {
				var tbi = tb.linkedBoxes[i];
				if ( lb != tbi ) {
					var ns = str+tbi.box2str();
					objType = tbi.parent.root.obj.__.nodeType;
		...
		for (var i in rootObjects) {
			for (var g in rootObjects[i].links)
				this.getRootPath(rootObjects[i].links[g], false, rootObjects[i].links[g].box2str())
		}
...
function linePoints() {
	...
	this.getCycles = function() {		
		var cycles = false;
		console.log("____");
		this.getRootPath = function(tb, lb, checkBox) {
			...
			for (var i in tbl) {
				var tbi = tbl[i];
				if ( lb != tbi ) {
					console.log(checkBox + " <> " + tbi); 
					if (checkBox == tbi) {
						cycles = true;
					}
					this.getRootPath(tbi, tb, checkBox)
				}
			}
		}
		
		console.log(this.lastBox.box2str());
		console.log(directedGraph[this.lastBox.box2str()]);
		
		this.getRootPath(this.lastBox.box2str(), false, this.curBox.box2str());
		return cycles;		
	}
...
function listLabel(o) {
	...
	this.renameObject = function(newName) {
		...
		function renameobjptr(str, newPtr) {
			...
			/*
			console.log(o);
			var npsplit = ptrSplit(newPtr);
			console.log(npsplit);
			var oldptrsplit = ptrSplit(o.ptr);
			console.log(oldptrsplit);
			
			if ( ! npsplit.length === oldptrsplit.length) {
				console.log("This is not supposed to happen");
			}
			
			
			console.log(typeof nsplit);
			console.log("be-for");
			console.log(nsplit.length);
			for ( var i = 0 ; i < nsplit.length ; i++) {
				//if ( nsplit[i] !== oldptrsplit[i]) {
				//	npsplit[i] = ptrSplit(o.ptr)[i];
				//}
			}
			...
			console.log(npsplit);
			return myforeignstring({id: o.id, ptr: ptrJoin(npsplit), index: o.index });
			*/
